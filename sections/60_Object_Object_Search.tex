...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \Procedure{objectObjectSearch}{$Q$,$C$} %\Comment{$M$ a matrix of a Loc's objects, $L$ an ordered list of objects to search for, $D$ is the pruning direction}
            ...
            \State{$W\leftarrow$ West Center point of $C$} 
            \State{$NW\leftarrow$ NorthWest Center point of $C$} 
            \For{Each $R$ in [$N, W, NW$]} \Comment{\textbf{R}eference diretion}
                \For{Each point $P$ in $Q$}
                    \State{$\theta$ $\leftarrow$ $\angle$ $R$$Q.x$$P$ }\Comment{Measured clockwise}
